...
//systemChat format ["%1",_randomLZArray];
//systemChat format ["%1",_spawnerPOS];
//systemChat format ["%1",_randomLZ];
//make sure the lz choosen is not the exact same one, loop through until they are different
...
if !(_HQPOS isEqualto _spawnerPOS) then {    
    ...
    //systemChat format ["TP Script"]; 
    [[west,"HQ"],"New LZ Placed"] remoteExec ["sideChat",-2];    
} else{[[west,"HQ"],"ARE TASK 1 Tight LZs Training Complete"] remoteExec ["sideChat",-2]};  
